...
import Motan1 from "../assets/motan-1.png";
import Motain2 from "../assets/motain-2.png";
import Motan3 from "../assets/motan-3.png";
import Motain4 from "../assets/motain-4.png";
import Motan6 from "../assets/motan-6.png";
import { AiOutlineArrowLeft } from "react-icons/ai"; // استيراد أيقونة السهم
...
const MotanLamar = () => {
  const hotelFeatures = ["حوالي 15 دقيقة مشيا", "توصيل مجاني للحرم 24 ساعه"];
const smallImages = [Motan1, Motain2, Motain4,Motan3, , Motan6]; // هنا عرفنا المتغير
  ...
  <Link
    to="/hotels"
    className="flex items-center gap-1 text-[#D39D19] font-bold text-[14px] md:text-[16px] lg:text-[16px] hover:text-[#B37A10]"
  >
         ...
         <div className="flex items-center gap-3">
           <div className="flex gap-[2px] text-[24px]">
             {Array(5).fill(0).map((_, idx) => (
               <TiStarFullOutline
                 key={idx}
                 className={`w-[24px] h-[24px] ${idx < 3 ? "text-[#D39D19]" : "text-[#D9D9D9]"}`}
               />
             ))}
           </div>
           <h1 className="text-[20px] text-[#333333] font-medium">فندق 3 نجوم</h1>
         </div>
