...
export default function SectionTwo() {
  ...
  return (
<div className="bg-[#E0DFDF] w-full flex flex-col items-center px-4 sm:px-6 md:px-16 py-12 sm:py-16 md:py-20 relative -translate-y-[10px] shadow-[0_8px_12px_rgba(0,0,0,0.25)]">
      <div className="w-full md:max-w-[1100px] flex flex-col-reverse md:flex-row items-center justify-center gap-8 md:gap-[100px] mt-10">
        <div className="relative flex justify-center items-center w-full md:w-[49%] mb-6 md:mb-0">
          <div 
            className="absolute bg-[#bcb9b2] shadow-lg z-0 rounded-[20px] w-[90%] sm:w-[440px] h-[250px] sm:h-[450px] translate-y-[15px]"
          ></div>
          <motion.img
            src={sectionTwoImg}
            alt="صورة الجانب الأيسر" 
            className="relative z-10 object-cover w-[90%] sm:w-[440px] h-[450px] rounded-[20px] translate-y-[15px]"
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.9 }}
          />
        </div>
        <div className="w-full md:w-[49%] flex flex-col items-end text-right space-y-4">
          <h2 className="text-[20px] sm:text-[40px] md:text-[40px] font-cairo font-bold text-[#30302F] leading-[1.2]">
            من نحن <span className="text-[#C8A44C]">؟</span>
          </h2>
          <p className="text-[16px] sm:text-[20px] md:text-[22px] font-cairo text-[#333] leading-[1.8] max-w-full sm:max-w-[440px] text-right">
          ...
          <ul className="space-y-2 text-[14px] sm:text-[18px] text-[#333333] leading-[1.8]">
            {[
